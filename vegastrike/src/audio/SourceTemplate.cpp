...
namespace Audio {
    ...
    SourceTemplate::SourceTemplate(const std::string &sound, VSFileSystem::VSFileType type, bool _looping) noexcept :
        soundName(sound),
        soundType(type),
        cosAngleRange(-1,-1),
        pfRadiusRatios(1,1),
    ...
    Range<Scalar> SourceTemplate::getAngleRange() const noexcept 
    { 
        return Range<Scalar>(Scalar(acos(cosAngleRange.min)), 
                             Scalar(acos(cosAngleRange.max))); 
    }
    
    void SourceTemplate::setAngleRange(Range<Scalar> r) noexcept 
    { 
