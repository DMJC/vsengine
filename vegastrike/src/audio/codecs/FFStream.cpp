...
//                if (  (0 != av_open_input_file(&pFormatCtx, npath.c_str(), NULL, BUFFER_SIZE, NULL))                
                if (  (0 != avformat_open_input(&pFormatCtx, npath.c_str(), NULL, NULL))
                    ||(0 >  avformat_find_stream_info(pFormatCtx, NULL))  )
...
/*                for (unsigned int i=0; (pCodecCtx==0) && (i < pFormatCtx->nb_streams); ++i)
                    if ((pFormatCtx->codec_type == CODEC_TYPE_AUDIO) && (streamIdx-- == 0))
                        pCodecCtx->codec_type = pFormatCtx->audio_codec;
...
//                if(avcodec_open(pCodecCtx, pCodec) < 0)
                ...
                case AV_SAMPLE_FMT_S24: fmt.bitsPerSample = 24;
                                     fmt.signedSamples = 1;
                                     break;
                #endif
                #ifdef SAMPLE_FMT_S32
                case AV_SAMPLE_FMT_S32: fmt.bitsPerSample = 32;
                                     fmt.signedSamples = 1;
                                     break;
                #endif
