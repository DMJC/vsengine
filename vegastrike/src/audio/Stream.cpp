...
namespace Audio {
    ...
    double Stream::getLength() noexcept(false)
    {
        return getLengthImpl();
    }

    double Stream::getPosition() const noexcept
    {
        return getPositionImpl();
    }

    void Stream::seek(double position) noexcept(false)
    {
        seekImpl(position);
    }

    unsigned int Stream::read(void *buffer, unsigned int bufferSize) noexcept(false)
    {
        void *rbuffer;
        void *rbufferEnd;
