...
extern "C" int _url_open( AVIOContext *h, const char *filename, int flags )
{
    ...
    if (f->OpenReadOnly( path, vstype ) > Ok) {
        ...
        h->buf_ptr = (unsigned char *) f;
        return 0;
    }
}

extern "C" int _url_close( AVIOContext *h )
{
    delete (VSFile*) (h->buf_ptr);
    return 0;
}

extern "C" int _url_read( AVIOContext *h, unsigned char *buf, int size )
{
    return ( (VSFile*) (h->buf_ptr) )->Read( buf, size );
}
...
extern "C" int _url_write( AVIOContext *h,
...
extern "C" offset_t _url_seek( AVIOContext *h, offset_t pos, int whence )
{
    if (whence != AVSEEK_SIZE) {
        ( (VSFile*) (h->buf_ptr) )->GoTo( long(pos) );
        return ( (VSFile*) (h->buf_ptr) )->GetPosition();
    } else {
        return ( (VSFile*) (h->buf_ptr) )->Size();
    }
}
...
void initLibraries()
{
    ...
    if (!initted) {
        initted = true;
        av_register_all();
/*        #if (LIBAVFORMAT_VERSION_MAJOR >= 53 && LIBAVFORMAT_VERSION_MAJOR <= 60)
